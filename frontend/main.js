...
async function addProduct() {
  ...
  try {
    ...
    // 7) Tabloyu güncelle, formu temizle, toast zaten gösterildi
    populateProductTable();
    resetProductForm();
  } catch {
...
function editProduct(idx) {
  ...
  //document.getElementById('description').value = kalem.description;
...
async function updateProduct(idx) {
  ...
  const curr = document.getElementById('currencyType').value;
  const disc = parseFloat(document.getElementById('productDiscount').value);
  ...
  kalem.stockname = document.getElementById('productSelection').value;
  kalem.quantity = qty;
  kalem.price = pr;
  kalem.currency = curr;
  kalem.discount = disc;
  //kalem.description = desc;
  populateProductTable();
  resetProductForm();
}
...
function resetProductForm() {
  ...
  const btn = document.querySelector("button[onclick^='updateProduct']");
  if (btn) { btn.textContent = 'Ekle'; btn.onclick = addProduct; }
  editingIndex = null;
}
